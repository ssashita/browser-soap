...
WSDL.prototype.invocationToDocumentXML = function(nm, params, namespace, xmlns) {
    //obj, name, namespace, xmlns
	var obj = {};
    obj[nm] = params;
	var self = this,
        parts = [],
        ...
        for (var name in obj) {
            ...
            parts.push(self.objectToXMLForDocument(child, name,''));
            parts.push(['</',ns,name,'>'].join(''));
        }

    return parts.join('');
}
WSDL.prototype.objectToXMLForDocument = function(obj, name) {
    var self = this,
        parts = [];
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            ...
            parts.push(self.objectToXMLForDocument(item, name));
        }
    }
